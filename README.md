...
| `04_sproc_setup.sql` | Stored Procedure（カスタムツール） |
| `agent_design.md` | Intelligence Agent設計ガイド |

## 💡 環境構築と利用方法

### Step 1-4: ツールの準備
1. `01_db_setup.sql` を実行してデータベース・テーブル・サンプルデータを作成
2. `02_sv_setup.sql` を参考にSnowsight GUIでSemantic Viewを作成
3. `03_rag_setup.sql` を実行してCortex Search Serviceを作成
4. `04_sproc_setup.sql` を実行してStored Procedureを作成

### Step 5: Marketplaceからデータ取得
5. Snowflake Marketplaceから「**Cortex Knowledge Extensions**」を取得
...
6. `agent_design.md` を参考にCortex Agentを作成し、以下のツールを登録：
...
### Step 7: Snowflake Intelligenceで利用
7. Snowflake Intelligenceから作成したCortex Agentを選択してチャット開始
